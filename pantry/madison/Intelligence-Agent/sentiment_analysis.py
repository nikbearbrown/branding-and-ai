...
class GPTAnalyzer:
 ...
 self.api_key = None
 self.enabled = False
 print("⚠ GPT API key not provided. GPT analysis is disabled.")
...
try:
 ...
 print("⚠ Running outside Colab or missing libraries. Cannot load Google Sheets.")
 processed_links = pd.DataFrame()
 drift_metrics = pd.DataFrame()
 competitor_analysis = pd.DataFrame()
except Exception as e:
 print(f"⚠ An error occurred during Colab/gspread setup: {e}")
 processed_links = pd.DataFrame()
 drift_metrics = pd.DataFrame()
 competitor_analysis = pd.DataFrame()
...
if not processed_links.empty:
 ...
 print(f"⚠ GPT FAILED: {analysis['primary_cause']} - The 401 error is causing this fallback.")
 print("-" * 40)

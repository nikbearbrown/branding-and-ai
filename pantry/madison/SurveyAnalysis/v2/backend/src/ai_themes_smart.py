...
try:
 ...
 logger.warning("⚠ sentence-transformers not available, will use TF-IDF fallback")
...
def extract_themes_smart(
 ...
 if not raw_themes:
 logger.warning("⚠ [SMART-THEMES] No themes extracted")
 return []
...
def _extract_raw_themes_from_batches(
 ...
 logger.warning(f"⚠ [SMART-THEMES] Batch {batch_num + 1} failed: {e}")
 continue

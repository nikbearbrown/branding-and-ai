...
def _retry_openai_call(call_func, max_retries=3, operation_name="OpenAI API call"):
 ...
 logger.warning(f"⚠ [AI] Connection error in {operation_name}, attempt {attempt + 1}/{max_retries}. Retrying in {wait_time}s... Error: {e}")
...
def _get_openai_client():
 ...
 logger.warning("⚠ [AI] OpenAI client initialization issue with proxies, trying alternative method")
...
def classify_sentiments(texts: List[str], batch_size: int = 40) -> List[str]:
 ...
 if not texts:
 logger.warning("⚠ No texts provided for sentiment classification")
 return []
 ...
 logger.warning(f"⚠ [AI] This may be due to network issues or OpenAI API being temporarily unavailable")
 ...
 logger.warning(f"⚠ [AI] OpenAI API rate limit reached. Consider reducing batch size or adding delays.")
 ...
 logger.warning(f"⚠ [AI] Falling back to neutral labels for batch {batch_num}")
...
def extract_weighted_keyphrases(texts: List[str], top_k: int = 20) -> List[Dict]:
 ...
 logger.warning(f"⚠ [AI] Smart batched approach failed: {e}, falling back to sampling")
...
Return ONLY valid JSON, no other text."""
 ...
 logger.warning("⚠ [AI] response_format not supported, using standard format")
 response = client.chat.completions.create(
 model="gpt-3.5-turbo",
 messages=[
 {"role": "system", "content": "You are a data analyst extracting key themes from customer feedback. Always return valid JSON."},
 ...
 logger.warning("⚠ [AI] No themes extracted from JSON response")
 
 except json.JSONDecodeError as e:
 logger.warning(f"⚠ [AI] JSON parsing failed: {e}, attempting alternative parsing")
 ...
 logger.warning("⚠ [AI] Failed to parse JSON, attempting text extraction")
 ...
 logger.warning("⚠ [AI] No themes extracted, returning empty list")
 return []
...
def extract_impact_terms(texts: List[str], top_k: int = 50) -> List[Dict]:
 ...
 if not texts:
 logger.warning("⚠ No texts provided for impact terms extraction")
 return []
...
Return ONLY valid JSON, no other text."""
 ...
 logger.warning("⚠ [AI] No impact terms extracted from JSON response")
 ...
 logger.warning("⚠ [AI] No impact terms extracted, returning empty list")
 return []

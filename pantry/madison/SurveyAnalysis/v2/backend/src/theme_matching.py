...
def _check_embeddings_availability():
 ...
 EMBEDDINGS_AVAILABLE = False
 logger.warning("⚠ Embeddings returned empty, using word-based matching")
 except (ImportError, RuntimeError) as e:
 EMBEDDINGS_AVAILABLE = False
 if "torch" in str(e).lower() or "pytorch" in str(e).lower() or "DLL" in str(e):
 logger.warning("⚠ PyTorch initialization failed (Windows compatibility issue). Using word-based matching fallback.")
 ...
 logger.warning(f"⚠ Embeddings not available: {e}. Using word-based matching fallback.")
 except Exception as e:
 EMBEDDINGS_AVAILABLE = False
 logger.warning(f"⚠ Embeddings check failed: {e}. Using word-based matching fallback.")
...
def match_theme_to_texts_semantic(
 ...
 logger.warning(f"⚠ Embedding count mismatch: {len(text_embeddings)} vs {len(texts)}")
 return _match_theme_to_texts_fallback(theme, texts)
 ...
 if "torch" in str(e).lower() or "pytorch" in str(e).lower() or "DLL" in str(e):
 EMBEDDINGS_AVAILABLE = False
 logger.debug(f"⚠ Disabling embeddings due to PyTorch error, using fallback for '{theme[:40]}'")
 else:
 logger.debug(f"⚠ Semantic matching failed for theme '{theme[:40]}': {e}, using fallback")
 return _match_theme_to_texts_fallback(theme, texts)
 except Exception as e:
 logger.debug(f"⚠ Semantic matching failed for theme '{theme[:40]}': {e}, using fallback")
 return _match_theme_to_texts_fallback(theme, texts)

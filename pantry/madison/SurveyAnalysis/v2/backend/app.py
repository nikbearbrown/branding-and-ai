...
async def analyze_sentiment_endpoint(
 ...
 import logging
 logging.warning(f"⚠ [APP] AI sentiment classification failed: {e}, falling back to VADER")
 df_proc = analyze_sentiment(df_raw, report["text_col"])
 else:
 import logging
...
async def extract_themes_endpoint(
 ...
 logging.warning("⚠ [APP] AI theme extraction returned empty results, falling back to YAKE/TF-IDF")
 except Exception as e:
 import logging
 logging.warning(f"⚠ [APP] AI theme extraction failed: {e}, falling back to YAKE/TF-IDF")
 pass
